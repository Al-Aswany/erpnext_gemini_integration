...
def mask_data(text, keywords_str):
    ...
    # Add common financial patterns (simple example)
    # A more robust solution would use regex for patterns like credit card numbers
    # For simplicity, adding specific keywords as requested
    financial_keywords = ["credit_card", "salary", "bank_account", "ssn"]
    keywords.extend([kw for kw in financial_keywords if kw not in keywords])

    masked_text = text
    for kw in keywords:
        # Basic case-insensitive replacement
        # More robust: Use regex with word boundaries (\b) to avoid partial matches
        # For simplicity, using simple replace as shown in requirement
        # Note: This simple replace might be too broad. Regex is better.
        # Example with regex (case-insensitive, whole word): 
        # import re
        # masked_text = re.sub(r"\b" + re.escape(kw) + r"\b", "****", masked_text, flags=re.IGNORECASE)
        
        # Using simple replace as per requirement example structure
        # This will replace substrings, e.g., "classification" -> "classi****tion" if "fica" is a keyword.
        # A better approach is needed for production.
        if kw in masked_text: # Simple check to avoid unnecessary replaces
             masked_text = masked_text.replace(kw, "****") 
...
def process_message(message, conversation_id=None, files=None, context=None):
    ...
    try:
        ...
        original_message = message # Keep original for logging if needed
        message = mask_data(message, sensitive_keywords)
        # --- End Masking ---
        ...
        # Log the interaction (using the masked message)
        message_id = gemini.log_interaction(
            message, # Log the masked message
            response, 
            conversation_id=conversation_id, 
            context=context_dict
        )
